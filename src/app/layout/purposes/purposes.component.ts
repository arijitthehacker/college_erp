...
export class PurposesComponent implements OnInit {
  ...
  getData() {
    ...
    this.http.getData(ApiUrl.list_property_purpose, obj).subscribe(res => {
  ...
  deleteData(data: any) {
    this.message.confirm(`delete this ${this.commonService.title}`).then(res => {
      if (res.isConfirmed) {
        ...
        this.http.putData(ApiUrl.managed_peroperty_purpose, obj).subscribe(() => {
  ...
  addEditModalOpen(data?: any) {
    const modalRef = this.modalService.show(AddPurposeComponent, {
